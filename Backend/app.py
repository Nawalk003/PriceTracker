...
def get_productsFromFlipkart(url, search_text):
    ...
    results = []
    containerClass = "DOjaWF gdgoEp"
    itemClass = "cPHDOP col-12-12"
    itemClassSmall="slAVV4"
    itemNameClass ="KzDlHZ" or "wjcEIp"
    itemImageClass = "DByuf4"
    itemUrlClass= "CGtC98" or "VJA3rP"
    priceClass = "Nx9bqj _4b5DiR" or "Nx9bqj"
    ...
    # itemsSmall=items[0].findAll("div", {"class": itemClassSmall})
    # print("itemSmall length of items is ", len(itemsSmall))
    ...
    for item in items :
        ...
        if len(urls) == 0:
            continue
        itemNames = item.findAll("div" or "a", {"class": itemNameClass})
        # print("name length is ", len(itemNames))
        prices = item.findAll("div", {"class": priceClass})
        price = prices[0].text
        price = price[1:]
        price = price.replace(",", "")
        # print("price length is ", len(price))
...
# def get_productsFromAmazon(search_text):
    query_string = "https://www.amazon.in/s?k="+ search_text +"&crid=X54XP4H4JYT0&sprefix="+ search_text + "%2Caps%2C214&ref=nb_sb_noss_1"
    results = []
    containerClass = "nt6sNV JxFEK3 _48O0EI"
    itemClass = "yKfJKb row"
    itemNameClass ="KzDlHZ"
    itemImageClass = "DByuf4"
    itemUrlClass= "CGtC98"
